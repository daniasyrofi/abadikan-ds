...
const bgActive = computed(() => isScrolled.value || showMega.value || mobileOpen.value)
const heroRootRef = ref<HTMLElement | null>(null)
const isSocialFixed = ref(true)
const SOCIAL_BAR_H = 82 // approximate social bar height in px
...
const pillClasses = computed(() => {
  ...
  if (!isScrolled.value && showMega.value) {
    // Mega open but not yet scrolled: full-width, white bg, no shrink
    return 'max-w-[1040px] bg-white border border-[var(--color-border)] rounded-none shadow-none'
  }
  if (showMega.value) {
    // Mega open + scrolled: pill width, flat all sides to connect panel
    return 'max-w-[720px] bg-white border border-[var(--color-border)] rounded-none shadow-none'
  }
  if (mobileOpen.value) {
    // Drawer open: always full width, flat (no rounding)
    return 'max-w-[1040px] bg-white border border-[var(--color-border)] rounded-none shadow-none'
  }
  // Scrolled: flat top (flush to viewport), rounded bottom
  return 'max-w-[720px] bg-white border border-[var(--color-border)] rounded-t-none rounded-b-[20px] shadow-sm'
})

function onScroll() {
  isScrolled.value = window.scrollY > 80
  // Release social bar from fixed when its natural in-flow position enters the viewport
  if (heroRootRef.value) {
    const heroBottom = heroRootRef.value.getBoundingClientRect().bottom
    isSocialFixed.value = heroBottom - SOCIAL_BAR_H > window.innerHeight
  }
}
onMounted(() => {
  window.addEventListener('scroll', onScroll, { passive: true })
  startCounters()
  onScroll() // initialize fixed state on first render
})
...
<template>
  <div class="hero-root" ref="heroRootRef">
    <!-- Radial glow overlay -->
    ...
    <div class="fixed top-0 left-0 right-0 z-[100]">
      ...
      >
        <div class="flex items-center justify-between px-5 py-[4px] max-w-[1040px] mx-auto">
          ...
          <div class="flex items-center gap-[4px]">
            <!-- Active tab: outer wrapper has border-b, inner has bg -->
            <div class="py-[4px] border-b border-white">
              <button class="px-[8px] py-[4px] bg-white/10 rounded-[4px] text-[14px] font-[600] text-white whitespace-nowrap hover:bg-white/15 transition-colors">Personal</button>
            </div>
            <!-- Inactive tab: no border-b on outer, no bg on inner -->
            <div class="py-[4px] border-b border-transparent">
              <button class="px-[8px] py-[4px] rounded-[4px] text-[14px] font-[600] text-white whitespace-nowrap hover:bg-white/10 transition-colors" @click="navigateTo('business')">Bisnis</button>
            </div>
          </div>
      ...
      <div
        ...
        :class="isScrolled && !mobileOpen ? 'md:px-4 py-0' : 'px-0 py-0'"
      >
      ...
      <template #center>
        <nav
          class="hidden md:flex h-full items-center justify-center gap-[4px] w-[244px]"
          :style="{
            color: bgActive ? 'var(--color-text-heading)' : 'var(--color-text-inverse)',
            transition: 'color var(--duration-slow)',
          }"
        ...
        >
          ...
          >
            ...
            >
              <div
                v-show="showMega"
                class="absolute top-[100%] left-[-1px] w-[calc(100%+2px)] bg-white rounded-b-[24px] shadow-[0_16px_40px_rgba(0,0,0,0.12)] border border-t-0 border-[var(--color-border)] p-5 pt-4 text-left cursor-default -z-10 origin-top"
              >
                <div class="grid grid-cols-3 gap-2 normal-case tracking-normal">
                  <a
                    href="#"
                    class="flex gap-3 items-start p-3 rounded-md hover:bg-[var(--color-bg-subtle)] group transition-colors"
                    @click.stop="navigateTo('consumer')"
                  >
                    <div class="text-[var(--color-primary)] mt-0.5 shrink-0" v-html="megaIcons.OnCall"></div>
                    <div>
                      <div class="text-[13px] font-[650] text-[var(--color-text-heading)] group-hover:text-[var(--color-primary)] transition-colors">
                        Undangan Digital
                      </div>
                      <div class="text-[12px] text-[var(--color-text-secondary)] mt-0.5 leading-snug">
                        Undangan website pernikahan yang elegan dan interaktif
                      </div>
                    </div>
                  </a>
                  <a
                    ...
                    @click.stop="navigateTo('consumer')"
                  >
                    <div class="text-[var(--color-primary)] mt-0.5 shrink-0" v-html="megaIcons.Status"></div>
                    <div>
                      <div class="text-[13px] font-[650] text-[var(--color-text-heading)] group-hover:text-[var(--color-primary)] transition-colors">
                        Buku Tamu Digital
                      </div>
                      <div class="text-[12px] text-[var(--color-text-secondary)] mt-0.5 leading-snug">
                        Kelola RSVP dan daftar hadir tamu secara otomatis
                      </div>
                    </div>
                  </a>
                  <a
                    ...
                    @click.stop="navigateTo('consumer')"
                  >
                    <div class="text-[var(--color-primary)] mt-0.5 shrink-0" v-html="megaIcons.Fire"></div>
                    <div>
                      <div class="text-[13px] font-[650] text-[var(--color-text-heading)] group-hover:text-[var(--color-primary)] transition-colors">
                        Kado Digital
                      </div>
                      <div class="text-[12px] text-[var(--color-text-secondary)] mt-0.5 leading-snug">
                        Terima kado dan amplop digital dari tamu undangan
                      </div>
                    </div>
                  </a>
                </div>
        ...
        <div class="flex items-center gap-[4px] justify-end">
          ...
          <div class="hidden md:flex items-center gap-[4px]">
            ...
            <Button
              ...
              :style="
                !bgActive
                  ? '--btn-bg: var(--color-text-inverse); --btn-text: var(--color-neutral); --btn-border: var(--color-text-inverse); --btn-hover-bg: rgba(255,255,255,0.72); --btn-hover-border: rgba(255,255,255,0.72);'
                  : ''
              "
              >Buat Undangan</Button
            >
          </div>
          <!-- Mobile: CTA + hamburger (<768px) -->
          <div class="flex md:hidden items-center gap-[4px]">
            <Button
              variant="default"
              size="sm"
              :style="
            ...
            <button
              ...
              :style="{ color: bgActive ? 'var(--color-text-heading)' : 'var(--color-text-inverse)' }"
    ...
    >
      <div
        v-show="mobileOpen"
        class="md:hidden fixed z-[99] left-0 right-0 bg-white border-b border-[var(--color-border)] shadow-lg px-5 py-3 flex flex-col"
        :class="isScrolled ? 'top-[64px]' : 'top-[104px]'"
      >
        <button
          v-for="item in [
            { label: 'Beranda', page: 'consumer' },
            { label: 'Produk', page: 'template' },
            { label: 'Panduan', page: 'tutorial' },
            { label: 'Reseller', page: 'business' },
          ]"
          :key="item.page"
          class="text-left px-3 py-2.5 rounded-lg text-[15px] font-[500] text-[var(--color-text-heading)] hover:bg-[var(--color-bg-subtle)] transition-colors"
          @click="navigateTo(item.page); mobileOpen = false"
        >
          {{ item.label }}
        </button>
        <div class="flex gap-2 mt-2 pt-3 border-t border-[var(--color-border-subtle)]">
          <Button variant="outline" size="md" class="flex-1">Login</Button>
          <Button variant="primary" size="md" class="flex-1">Buat Undangan</Button>
        </div>
      </div>
    </transition>
    ...
    <div class="hero-body">
      <div class="hero-center">
        ...
        <div class="hero-overline">
          <span>Satu Platform, Sejuta Cerita</span>
        </div>

        <!-- H1 -->
        <h1 class="hero-h1">
          Abadikan Cerita Cintamu.
        </h1>

        <!-- Subtitle -->
        <p class="hero-sub">
          Bikin undangan pernikahan yang memorable, estetik, dan penuh makna.
        </p>
        ...
        <div class="hero-ctas">
          <Button
            variant="default"
            size="lg"
            style="--btn-bg: var(--color-text-inverse); --btn-text: var(--color-neutral); --btn-border: var(--color-text-inverse); --btn-hover-bg: rgba(255,255,255,0.72); --btn-hover-border: rgba(255,255,255,0.72); font-weight: 600;"
          >
            Mulai Sekarang →
          </Button>
          <Button
        ...
        <!-- Trust line -->
        <p class="hero-trust">
          Dipercaya 12.000+ pasangan · 50.000+ tamu diundang
        </p>

      </div>
    </div>

    <!-- Spacer placeholder when social bar is fixed (prevents hero collapsing) -->
    <div v-if="isSocialFixed" class="social-bar-spacer" aria-hidden="true" />

    <!-- Social proof bar — fixed until natural position is reached -->
    <div :class="['social-bar', { 'social-bar--fixed': isSocialFixed }]">
...
.hero-root {
  ...
  min-height: 100vh;
  background:
    radial-gradient(ellipse 120% 50% at 50% -10%, oklch(0.35 0.005 80 / 0.4), transparent),
    oklch(0.13 0.008 80);
  overflow-x: hidden;
...
.hero-glow {
  ...
  inset: 0;
  pointer-events: none;
  background: radial-gradient(ellipse 60% 30% at 50% 0%, oklch(0.4 0.005 80 / 0.12), transparent);
  z-index: 0;
}
...
/* H1 */
.hero-h1 {
  font-size: clamp(2.2rem, 4.5vw + 0.5rem, 3.75rem);
  font-family: var(--font-display);
  font-weight: 800;
...
/* ── Mockup section (outside hero-root) ───────────────────────────────────── */
.hero-mockup-section {
  display: flex;
  justify-content: center;
...
/* When social bar is fixed (before reaching natural position) */
.social-bar--fixed {
  position: fixed;
  bottom: 0;
  left: 0;
...
/* Spacer that holds the hero height when bar is fixed */
.social-bar-spacer {
  height: 82px;
  flex-shrink: 0;
}
