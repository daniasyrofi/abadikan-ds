...
function show() {
  ...
  showTimeout = setTimeout(() => { isVisible.value = true }, props.delay)
}

function hide() {
  if (showTimeout) { clearTimeout(showTimeout); showTimeout = null }
  hideTimeout = setTimeout(() => { isVisible.value = false }, 100)
}
...
const arrowClasses: Record<Placement, string> = {
  ...
  bottom: 'bottom-full left-1/2 -translate-x-1/2 border-b-[--color-neutral] border-x-transparent border-t-transparent border-4',
  left:   'left-full top-1/2 -translate-y-1/2 border-l-[--color-neutral] border-y-transparent border-r-transparent border-4',
  right:  'right-full top-1/2 -translate-y-1/2 border-r-[--color-neutral] border-y-transparent border-l-transparent border-4',
}
